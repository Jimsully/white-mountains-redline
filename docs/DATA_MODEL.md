...
## `trail_segment_api`
Read-only REST projection for the app repository. It returns the minimal public rendering fields plus GeoJSON LineString coordinates derived in SQL. Migration 013 hardens it as an owner-rights, `security_barrier` public projection with explicit verified plus human-verified predicates; migration 014 removes internal provenance/source/review fields from the browser-visible column set. Browser roles may select this projection only; they must not directly select `trails` or `trail_segments`.
