...
Accepted evidence remains evidence until the owner explicitly invokes confirmation. M7D-C application integration is implemented locally: an authenticated SSR repository uses only the two M7D-B RPCs, and the account action submits only the opaque evidence UUID. No schema or relationship changed for M7D-C. Migrations 011-013 remain unapplied live. Disposable local Supabase runtime acceptance for the M7D database-backed flow and Migration 013 has passed; production predeploy and deployment checks remain outstanding.
