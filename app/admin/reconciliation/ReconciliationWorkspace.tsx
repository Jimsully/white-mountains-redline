...
export function ReconciliationWorkspace({ artifact }: Props) {
  ...
  const [decisions, setDecisions] = useState<Record<string, ReconciliationDecision>>(() => {
    ...
    const stored = window.localStorage.getItem("white-mountains-redline-reconciliation-decisions");
    return stored ? JSON.parse(stored) as Record<string, ReconciliationDecision> : {};
  });
  ...
  function saveDecision(decision: ReconciliationDecisionValue, candidate?: ReconciliationCandidate) {
    if (!selected) return;
    const nextDecision: ReconciliationDecision = {
      inventoryItemKey: selected.item.itemKey,
      selectedCandidateNormalizedName: candidate?.sourceTrailNormalizedName,
      selectedSourceFeatureIds: candidate?.evidence.sourceFeatureIds ?? [],
      decision,
      reviewTimestamp: new Date().toISOString(),
      notes: notes.trim() || undefined,
    };
    const next = { ...decisions, [selected.item.itemKey]: nextDecision };
    setDecisions(next);
    window.localStorage.setItem("white-mountains-redline-reconciliation-decisions", JSON.stringify(next));
  }
    ...
    <main className="reconShell">
      ...
      <div className="reconGrid">
        <aside className="reconList">
          ...
          {visible.map((result) => (
            <button key={result.item.itemKey} type="button" className={selected?.item.itemKey === result.item.itemKey ? "reconItem active" : "reconItem"} onClick={() => setSelectedKey(result.item.itemKey)}>
        ...
        <section className="reconDetail">
          {selected ? <Detail result={selected} decision={decisions[selected.item.itemKey]} onAccept={(candidate) => saveDecision("accepted", candidate)} onReject={(candidate) => saveDecision("rejected", candidate)} onNeedsReview={() => saveDecision("needs_review", selectedCandidate)} onClear={clearDecision} notes={notes} onNotesChange={setNotes} /> : null}
        </section>
...
function Detail({ result, decision, onAccept, onReject, onNeedsReview, onClear, notes, onNotesChange }: { result: ReconciliationItemResult; decision?: ReconciliationDecision; onAccept: (candidate: ReconciliationCandidate) => void; onReject: (candidate: ReconciliationCandidate) => void; onNeedsReview: () => void; onClear: () => void; notes: string; onNotesChange: (value: string) => void }) {
  return <>
    ...
    <div className="candidateStack">
      {result.candidates.map((candidate) => <article key={candidate.sourceTrailNormalizedName} className="candidateCard"><div><strong>{candidate.sourceTrailDisplayName}</strong><span>{candidate.score}/100</span></div><p>{candidate.evidence.reasons.join("; ")}</p><dl><div><dt>Feature count</dt><dd>{candidate.evidence.sourceFeatureCount}</dd></div><div><dt>GIS miles</dt><dd>{candidate.evidence.sourceGisMiles}</dd></div><div><dt>Source IDs</dt><dd>{candidate.evidence.sourceFeatureIds.join(", ")}</dd></div></dl><button type="button" onClick={() => onAccept(candidate)}>Accept candidate</button><button type="button" onClick={() => onReject(candidate)}>Reject candidate</button></article>)}
    </div>
  </>;
}
