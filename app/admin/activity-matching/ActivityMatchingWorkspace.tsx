...
export function ActivityMatchingWorkspace({ artifact }: Props) {
  ...
  const selectedMatch = filter === "unmatched_activities" ? undefined : visibleMatches.find((match) => match.key === selectedMatchKey) ?? visibleMatches[0];
  ...
  return <main className="activityShell">
    ...
    <div className="activityGrid">
      ...
      <section className="reconMap">{selectedMatch && selectedActivity && selectedSegment ? <ActivityPreview match={selectedMatch} activityCoordinates={selectedActivity.trace.geometry.coordinates} segmentCoordinates={selectedSegment.geometry.coordinates} maximumInterpolatedActivityEdgeMeters={artifact.config.maximumInterpolatedActivityEdgeMeters} /> : null}</section>
    </div>
  </main>;
}
...
function ActivityPreview({ match, activityCoordinates, segmentCoordinates, maximumInterpolatedActivityEdgeMeters }: { match: SegmentMatchCandidate; activityCoordinates: number[][][]; segmentCoordinates: number[][]; maximumInterpolatedActivityEdgeMeters: number }) {
  ...
  return <svg viewBox="0 0 360 360" role="img" aria-label="Schematic activity matching evidence"><rect width="360" height="360" fill="#d6d4ca" />{trustedLines.map((line, index) => <polyline key={`activity-${index}`} points={line.map(project).join(" ")} fill="none" stroke="#345f7c" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" opacity="0.72" />)}{trustedTrace.ignoredGaps.map((gap, index) => <g key={`gap-${index}`}><line x1={Number(project(gap.start).split(",")[0])} y1={Number(project(gap.start).split(",")[1])} x2={Number(project(gap.end).split(",")[0])} y2={Number(project(gap.end).split(",")[1])} stroke="#345f7c" strokeWidth="2" strokeDasharray="5 6" opacity="0.35" /><circle cx={Number(project(gap.start).split(",")[0])} cy={Number(project(gap.start).split(",")[1])} r="3" fill="#345f7c" /><circle cx={Number(project(gap.end).split(",")[0])} cy={Number(project(gap.end).split(",")[1])} r="3" fill="#345f7c" /></g>)}<polyline points={segmentCoordinates.map(project).join(" ")} fill="none" stroke="#d94a3a" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" /><circle cx={Number(project(segmentCoordinates[0]).split(",")[0])} cy={Number(project(segmentCoordinates[0]).split(",")[1])} r="6" fill="#161a18" /><circle cx={Number(project(segmentCoordinates[segmentCoordinates.length - 1]).split(",")[0])} cy={Number(project(segmentCoordinates[segmentCoordinates.length - 1]).split(",")[1])} r="6" fill="#161a18" /><text x="18" y="28" fontSize="12" fill="#161a18">GPS EVIDENCE * {match.classification.replace(/_/g, " ").toUpperCase()}</text>{trustedTrace.ignoredGaps.length ? <text x="18" y="348" fontSize="11" fill="#345f7c">DASHED LINES ARE IGNORED EVIDENCE GAPS</text> : null}</svg>;
}